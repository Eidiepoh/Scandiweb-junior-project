    ...
    goToPrevImage = () => {
        if (this.state.images && this.state.images.length) {
          this.setState(prevState => ({
            currentImageIndex: prevState.currentImageIndex > 0 ? prevState.currentImageIndex - 1 : prevState.images.length - 1
          }));
        }
      };
    ...
    render() {
        return(
            <div className={`product-card-images ${this.state.componentStyle}`}>
                ...
                src={this.state.images[this.state.currentImageIndex]} alt="product"/>
